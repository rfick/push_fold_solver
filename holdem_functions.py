...
def print_results(hole_cards, winner_list, result_histograms):
    ...
    for index, hole_card in enumerate(hole_cards):
        ...
        if hole_card == (None, None):
            print("(?, ?) : {}".format(winning_percentage))
        else:
            print("{}: {}".format(hole_card, winning_percentage))
    print("Ties: {}\n".format(float(winner_list[0]) / float_iterations))
    for player_index, histogram in enumerate(result_histograms):
        print("Player{} Histogram: ".format(str(player_index + 1)))
        for index, elem in enumerate(histogram):
            print("{}: {}".format(hand_rankings[index],  float(elem) / float_iterations))
        print("")
